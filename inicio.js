...
function carregarDestaques() {
    ...
    container.innerHTML = destaques.map(produto => `
        <div class="produto-card" onclick="abrirProduto(${produto.id})">
            ...
            <span class="produto-card-avaliacao"><span class="estrela">★</span>${produto.avaliacao}</span>
            <div class="produto-imagem-placeholder">${imagemProdutoHTML(produto)}</div>
            <button class="btn-add-rapido" onclick="event.stopPropagation(); adicionarRapido(${produto.id})" aria-label="Adicionar">+</button>
