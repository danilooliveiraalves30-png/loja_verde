...
function abrirProduto(id) {
    ...
    estado.telaAntesDoProduto = estado.telaAtual;
    estado.produtoAberto = true;
    ...
    document.querySelector('.navegacao-inferior').classList.add('hidden');
    document.getElementById('mini-carrinho').classList.add('hidden');
    window.scrollTo(0, 0);
}

function voltarDoProduto() {
    estado.produtoAberto = false;
    document.querySelector('.navegacao-inferior').classList.remove('hidden');
    navegarPara(estado.telaAntesDoProduto || 'inicio');
}
...
function atualizarPrecoBotaoAdicionar() {
    ...
    const total = estado.produtoSelecionado.preco * estado.quantidadeProduto;
    document.getElementById('btn-adicionar-preco').textContent = `R$ ${total.toFixed(2).replace('.', ',')}`;
}
...
function adicionarRapido(id) {
    const produto = api.obterProduto(id);
    if (!produto) return;
    api.adicionarItem(id, 1);
    mostrarToast(`${produto.nome} adicionado ao carrinho!`);
    atualizarMiniCarrinho();
    if (estado.telaAtual === 'carrinho') carregarCarrinho();
}
